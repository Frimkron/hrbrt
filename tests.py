...
class TestDocument(unittest.TestCase):
	...
	def test_parse_allows_multiple_sections(self):
		...
		hps.Section.parse.side_effect = make_parse({"s":secitr.next})
...
class TestHeading(unittest.TestCase):
	...
	def test_parse_expects_first_headingmarker(self):
		...
		self.assertEquals( 1, hps.HeadingMarker.parse.call_count )
...
class TestChoiceDescription(unittest.TestCase):
	...
	def test_parse_returns_populated_choicedescription(self):
		...
		hps.ChoiceDescNewline.parse.side_effect = make_parse({"n":hps.ChoiceDescNewline("foo"),"N":hps.ChoiceDescNewline("bar")})
	...
	def test_parse_expects_part(self):
		...
		self.assertEquals( 1, hps.ChoiceDescPart.parse.call_count )
...
class TestHrbrtIO(unittest.TestCase):
	...
	def test_read_invokes_parse_with_input_obj(self):
		...
		self.assertEquals( 1, len(hps.Document.parse.call_args_list) )
		self.assertEquals( 0, len(hps.Document.parse.call_args[1]) )
		self.assertEquals( 1, len(hps.Document.parse.call_args[0]) )
	...
	def test_read_returns_parse_result(self):
		...
		s = io.BytesIO("test")
		self.assertEquals(m, hio.HrbrtIO.read(s) )
